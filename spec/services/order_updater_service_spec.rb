...
RSpec.describe OrderUpdaterService do
  before do
    ...
    @service = OrderUpdaterService.new(@order)
  end

  context '#update_status' do
    it 'updates status to processing' do
      expect { @service.update_status('processing') }.to change { @order.status }.from('pending').to('processing')
    ...
    it 'updates status to confirmed if all items are present' do
      expect { @service.update_status('confirmed') }.to change { @order.status }.from('pending').to('confirmed')
    ...
    it 'raises error if status is confirmed but not all items are present' do
      ...
      expect { @service.update_status('confirmed') }.to raise_error("O pedido não pôde ser confirmado, pois não foi separado!")
    end

    it 'updates status to en_route if payment is paid' do
      @order.update(payment_status: 'paid')
      expect { @service.update_status('en_route') }.to change { @order.status }.from('pending').to('en_route')
    ...
    it 'raises error if status is en_route but payment is not paid' do
      expect { @service.update_status('en_route') }.to raise_error("O pedido não pôde ser colocado em rota de entrega, pois o pagamento ainda não confirmado!")
    end

    it 'updates status to ready_for_pickup if payment is paid' do
      @order.update(payment_status: 'paid')
      expect { @service.update_status('ready_for_pickup') }.to change { @order.status }.from('pending').to('ready_for_pickup')
    ...
    it 'raises error if status is ready_for_pickup but payment is not paid' do
      expect { @service.update_status('ready_for_pickup') }.to raise_error("O pedido não está disponível para retirada, pois o pagamento ainda não confirmado!")
    end
  end

  context '#update_payment_status' do
    it 'updates payment status to paid' do
      expect { @service.update_payment_status('paid') }.to change { @order.payment_status }.from('unsettled').to('paid')
      expect { puts "Pagamento recebido!" }.to output.to_stdout
    end

    it 'updates payment status to cancelled' do
      expect { @service.update_payment_status('cancelled') }.to change { @order.payment_status }.from('unsettled').to('cancelled')
